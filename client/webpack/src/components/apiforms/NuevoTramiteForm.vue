<template>
  <v-card text-center>
      ...
      <v-row>
        <v-col cols="12" xs="12" sm="12" md="12" lg="12" xl="12">
          ...
          <span v-else class="font-weight-thin body-2">
            <span v-if="data.origin !== 'user' && data.origin !=='approver'">Registre los trámites solicitados por los matriculados</span>
          </span>
        </v-col>
    ...
    <v-card-actions>
      ...
      <v-btn v-else color="blue darken-1" text @click="updateTramite(data.form)" :disabled="working || ( data.origin === 'approver' && !data.form.file)" :loading="working">
...
export default {
  ...
  created () {
    ...
    if (this.data.origin !== 'approver' && this.user.user && this.user.user.custom_fields) {
  ...
  computed: {
    ...
    _tiposTramites () {
      console.log('tiposTramites', process.env.NODE_ENV)
      if (process.env.NODE_ENV === 'development'){
        this.tiposTramites.push("Item de Prueba")
      }
    ...
    matriculados(val){
      if(val.payload){
        if (this.data.form.userId === 2) {
          ...
          val.payload.push(item)
        }
        this.matriculadosResult = val.payload;
  ...
  methods: {
    ...
    createTramite(item){
      ...
      if (item.matriculado[0]) {
        item.matriculado = item.matriculado[0]
      } else {
        item.matriculado = {
          ID: this.user.user.id,
          first_name: this.user.user.firstName,
          user_email: this.user.user.custom_fields.user_email,
          documento_nro: this.user.user.custom_fields.documento_nro,
          matricula: 0
        }
      }

      console.log('nuevo tramite form', item, this.user.user)
    ...
    updateTramite(item){
      ...
      switch (this.data.origin) {
        case 'user':
          item.status = 0 // Estado "PENDIENTE DE REVISION"
          break
        case 'approver':
          item.status = 2 // Estado "Completado"
      }

      item.matriculado = this.matriculadosResult.filter((matriculado) => {
        if(matriculado.ID === this.data.form.userId) return matriculado
      })
